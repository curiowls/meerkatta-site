...
- Capture: a 12-second real-app demonstration replaces the two-phone mockup. Native playback controls, captions, a poster, and a text description are included. No audio or automatic looping.
...
`public/media/capture-story.mp4` uses the existing approved app-preview source segments in the MeerKatta product repository: `marketing/app-store/preview-build-122/public/{lock-capture.mp4,capture.mp4,processing.mp4,result.png}`. Source recordings are `MK Lockscreen Capture.MP4` and `MeerKatta_mainflow.mov`. Sequence: 2 seconds Lock Screen, 5 seconds capture, 2 seconds processing, 3 seconds result. Pauses were shortened; this is not a speed benchmark. Output: 540×1170, 30 fps, H.264, 12 seconds, silent, approximately 256 KB.
