...
export function WorkStory({compact,example='team'}){
 ...
 return <AbsoluteFill style={{background:'#f2ede4',padding:compact?18:28,fontFamily:'Inter, sans-serif',color:'#302b26',borderRadius:24}}>
  <div style={{display:'flex',justifyContent:'space-between',fontSize:compact?12:13,color:'#786653',marginBottom:16}}><span>YOUR WORK, STILL OPEN</span><span>Illustrative demo</span></div>
  <div style={{background:'#fffdf8',border:'1px solid #d3c8b9',borderRadius:16,overflow:'hidden'}}>
   <div style={{padding:'16px 18px',borderBottom:'1px solid #e8dfd2',display:'flex',gap:14,alignItems:'center'}}><span style={{color:'#b4a592',letterSpacing:3}}>●●●</span><span style={{fontSize:compact?13:15}}>{e.context}</span></div>
   <div style={{padding:compact?18:24}}>
    <div style={{fontSize:13,color:'#8a7b69',marginBottom:16}}>Continue the conversation</div>
    <div style={{minHeight:compact?240:215,padding:18,border:'1.5px solid #ab8d70',borderRadius:12,background:inserted?'#fffaf1':'#fffdf8',position:'relative'}}>
     <div style={{fontSize:compact?19:21,lineHeight:1.55,whiteSpace:'pre-line',opacity:inserted?interpolate(f,[300,310],[0,1],{extrapolateLeft:'clamp',extrapolateRight:'clamp'}):1}}>{inserted?e.result:<span style={{color:'#9c9081'}}>{e.hint}<span style={{color:'#72462c'}}> ▎</span></span>}</div>
    </div>
    <div style={{fontSize:12,color:'#8a7b69',marginTop:12,textAlign:'right'}}>{inserted?'Inserted · Ready for your review':'Your cursor stays here'}</div>
   </div>
  </div>
  <div style={{marginTop:18,display:'flex',gap:12,alignItems:'center',color:'#72462c'}}>
   ...
   <span style={{fontSize:15}}>{inserted?'Your words, ready to use.':processing?'MeerKatta is shaping your words…':listening?'MeerKatta is listening…':'Ready when you are.'}</span>
  </div>
  <div style={{fontSize:compact?16:17,lineHeight:1.5,color:'#776653',marginTop:8,minHeight:compact?96:75}}>{listening||processing?<><span style={{fontSize:11,textTransform:'uppercase'}}>You say · </span>{spoken}</>:inserted?'The text is in your app. Review it, then keep going.':'Hold → speak → release. No change of workspace.'}</div>
  <div style={{marginTop:'auto',display:'flex',alignItems:'center',gap:16,borderTop:'1px solid #d9cebf',paddingTop:12}}>
   <svg width="100" height="66" viewBox="0 0 100 66" aria-hidden="true" style={{flexShrink:0,color:'#72462c'}}>
    ...
    <g transform={`translate(0 ${press*9})`}><rect x="10" y="9" width="80" height="36" rx="8" fill={listening?'#e5d5bd':'#f7f2e9'} stroke="currentColor" strokeWidth="1.7"/><text x="50" y="32" textAnchor="middle" fontFamily="Inter, sans-serif" fontSize="13" fill="currentColor">Hotkey</text></g>
   </svg>
   <div><div style={{fontSize:compact?17:19,fontWeight:500}}>{phase}</div><div style={{fontSize:12,color:'#8a7b69',marginTop:5}}>{listening?'Pressed · listening':processing?'Released · processing':inserted?'Released · inserted':'Released · ready'}</div></div>
  </div>
 </AbsoluteFill>;
}
